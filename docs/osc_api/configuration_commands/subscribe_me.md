...
```
/sio/cfg/osc/subscribers/subMe
```

Adds the source endpoint to the list of subscribers that receive report messages from the node.

### Arguments

- Arg 0 (OPTIONAL - `int` | `float`): Port number of the subscriber
...
The device will send OSC report messages (e.g., digital input changes) to all registered subscribers. Returns an error if the subscriber is already registered or
